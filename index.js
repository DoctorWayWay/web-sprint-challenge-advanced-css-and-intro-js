...
function get20s(array) {
  let artistsFromCentury20 = [];
  for (let i = 0; i < array.length; i++) {
    if (
      parseInt(array[i].years.split(" ", 1)) >= 1900 &&
      parseInt(
        array[i].years.split(" ")[array[i].years.split(" ").length - 1]
      ) <= 2000
    ) {
      artistsFromCentury20.push(array[i].name);
    }
  }
  return artistsFromCentury20;
}

// console.log(parseInt(artists[0].years.split(" ")));
// console.log(
//   parseInt(artists[0].years.split(" ")[artists[0].years.split(" ").length - 1])
// );
...
function addArtist(array) {
  array.push({
    ...
    bio: "Born in the sunny state called California, the aspiring American web developer continues to hone his skills. Eager to be better, he pushes himself to learn new things and rinse and repeat concepts he isn't familiar with yet.",
  });
  return array;
}
...
function lotsOfArt(array) {
  const paintingInBulkArtists = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].paintings > 100) {
      paintingInBulkArtists.push(array[i].name);
    }
  }
  return paintingInBulkArtists;
}
